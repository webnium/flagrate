...
return a};e.insert=function(a,b){if("string"===typeof b||"number"===typeof b||b instanceof HTMLElement)b={bottom:b};var c,d,m;for(m in b)if(c=b[m],m=m.toLowerCase(),d=e._insertionTranslation[m],c instanceof HTMLElement)d(a,c);else{"string"!==typeof c&&(c=c.toString(10));var f=new e;f.innerHTML=c;"top"!==m&&"after"!==m||(void 0).reverse();for(c=0;c<f.childNodes.length;c++)d(a,f.childNodes[c])}return a};e.insertText=function(a,b){if("string"===typeof b||"number"===typeof b)b={bottom:b};var c,d,f;for(f in b)c=
...
b);l(c,this);c.addClassName(f.className+" "+f.className+"-radio");a.icon&&(c.addClassName(f.className+"-icon"),c.setStyle({backgroundImage:"url("+a.icon+")"}));c._input=new e("input",{id:b,type:"radio",name:a.name});c.insert({top:new e});c.insert({top:c._input});a.isChecked&&c.check();a.isDisabled&&c.disable();return c};u.idCounter=0;u.prototype={disable:function(){this.addClassName(f.className+"-radio-disabled");this._input.writeAttribute("disabled",!0);return this},enable:function(){this.removeClassName(f.className+
...
null;a=new e("div",b);l(a,this);a.addClassName(f.className+" "+f.className+"-progress");a._updateProgress();return a};z.prototype={getValue:function(){return this.value},setValue:function(a){if("number"!==typeof a)return this;this.value=Math.max(0,Math.min(this.max,a));return this._updateProgress()},_updateProgress:function(){var a=Math.max(0,Math.min(100,100*(this.value/this.max)));this.update((new e).setStyle({width:a+"%"}));return this}};(f.Slider=function(a){a=a||{};var b=new z(a);l(b,this);b.addClassName(f.className+
...
onClick:function(){window.webkitNotifications.requestPermission()}.bind(this)}));return this}).prototype={create:function(a){a=a||{};if(!1===this.disableDesktopNotify&&!0===this.createDesktopNotify(a))return this;var b=a.title||this.title,c=a.message||a.body||a.content||a.text||null,d=a.onClick||null,m=a.onClose||null,g="undefined"!==typeof a.timeout?a.timeout:this.timeout,h=!0,n;a=this.hMargin;var l=this.vMargin,k=new e("div",{"class":this.className});k.addClassName(f.className+" "+f.className+"-notify");
(new e("div",{"class":"title"})).insertText(b).insertTo(k);(new e("div",{"class":"text"})).insertText(c).insertTo(k);(new e("div",{"class":"close"})).update("&#xd7;").insertTo(k).addEventListener("click",function(a){a.stopPropagation();a.preventDefault();h&&r()},!1);k.style.display="none";k.style.position="fixed";k.style[this.hAlign]=a+"px";k.style[this.vAlign]=l+"px";null===d?k.addEventListener("click",function(a){r()}):(k.style.cursor="pointer",k.addEventListener("click",function(a){a.stopPropagation();
a.preventDefault();d();r()}));this.target.appendChild(k);k.style.display="block";setTimeout(function(){k.style.opacity=1},10);if(0!==g){var p=function(){h&&r()};n=setTimeout(p,1E3*g);k.addEventListener("mouseover",function(){clearTimeout(n);n=setTimeout(p,1E3*g)})}var r=function(){h=!1;k.style.opacity=0;null!==m&&m();setTimeout(function(){this.target.removeChild(k);this.notifies.splice(this.notifies.indexOf(k),1);this.positioner()}.bind(this),300)}.bind(this);this.notifies.push(k);this.positioner();
...
!0===d.isSelected&&this.select(d);for(c=0;c<r;c++)m=this.cols[c],g=d.cell[m.key]||(d.cell[m.key]={}),g._td||(g._td=new e("td")),g._td.insertTo(d._tr),g.id&&g._td.writeAttribute("id",g.id),g.className&&g._td.writeAttribute("class",g.className),g.attribute&&g._td.writeAttribute(g.attribute),g.style&&g._td.setStyle(g.style),m.align&&(g._td.style.textAlign=m.align),g._td.addClassName(m._id),g._div||(g._div=new e),g._div.insertTo(g._td),g.text&&g._div.updateText(g.text),g.html&&g._div.update(g.html),g.element&&
g._div.update(g.element),g.icon&&(g._div.addClassName(f.className+"-icon"),g._div.setStyle({backgroundImage:"url("+g.icon+")"})),g.onClick&&(g._td.addClassName(f.className+"-grid-cell-clickable"),g._td.onclick=this._createCellOnClickHandler(this,g)),g.onDblClick&&(g._td.ondblclick=this._createCellOnDblClickHandler(this,g)),g.postProcess&&g.postProcess(g._td);d._last||(d._last=new e("td",{"class":this._id+"-col-last"}));d._last.insertTo(d._tr);if(this.pagination){if(b<l||b>=k)continue;++h}d.menuItems&&
(d._last.addClassName(f.className+"-grid-cell-menu"),d._menu&&d._menu.remove(),d._menu=new w({target:d._tr,items:d.menuItems}),d._last.onclick=this._createLastRowOnClickHandler(this,d));d._tr.insertTo(this._tbody);d.postProcess&&d.postProcess(d._tr);this.postProcessOfRow&&this.postProcessOfRow(d._tr)}this.pagination&&(this._pager.getElementByKey("rn").updateText(l+1+" - "+(l+h)+" / "+p),this._pager.getElementByKey("num").updateText(this.pagePosition+1+" / "+n));!1===this.disableResize&&(this.fill&&
